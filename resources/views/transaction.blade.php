                    ...
                    <table id="ordersTable" class="table align-middle table-hover text-center">
                        ...
                        <tbody>
                            @foreach ($orders as $order)
                                ...
                                <tr>
                                    <td><span class="fw-bold text-primary">#{{ $order['order_number'] }}</span>
                                        <a class="view-details-link" data-bs-toggle="modal"
                                            data-bs-target="#orderDetailModal{{ $modalOrderId }}">
                                            <i class="bi bi-info-circle me-1"></i>View Details
                                        </a>
                                    </td>
                                    <td>
                                        <span
                                            class="small fw-bold">{{ strtoupper($order['payment_method']) }}</span><br>
                                        <span
                                            class="{{ strtolower($order['payment_status']) === 'paid' ? 'text-paid' : 'text-notpaid' }}">
                                            {{ strtolower($order['payment_status']) === 'paid' ? 'PAID' : 'NOT PAID' }}
                                        </span>
                                        @if ($order['payment_proof'])
                                            <a class="view-proof-link" data-bs-toggle="modal"
                                                data-bs-target="#proofModal{{ $modalOrderId }}">
                                ...
                                <!-- Order Details Modal -->
                                <div class="modal fade" id="orderDetailModal{{ $modalOrderId }}" tabindex="-1"
                                    aria-hidden="true">
                                            ...
                                            <div class="modal-body p-4">
                                                        ...
                                                        <div
                                                            ...
                                                            <div>
                                                                ...
                                                                <small class="text-muted">Unit Price:
                                                                    ₱{{ number_format($p['unit_price'], 2) }}</small>
                                                            </div>
                                                ...
                                                <div class="card bg-light border-0 p-3">
                                                    <div class="d-flex justify-content-between mb-1">
                                                        <span>Subtotal</span><span>₱{{ number_format($totalPrice, 2) }}</span>
                                                    </div>
                                                    <div class="d-flex justify-content-between mb-1"><span>Shipping
                                                            Fee</span><span>₱0.00</span></div>
                                                    <hr>
                                                    <div class="d-flex justify-content-between fw-bold fs-5"><span>Total
                                                            Amount</span><span
                                                            class="text-success">₱{{ number_format($totalPrice, 2) }}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <!-- Payment Proof Modal -->
                                @if ($order['payment_proof'])
                                    <div class="modal fade" id="proofModal{{ $modalOrderId }}" tabindex="-1"
                                        aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content border-0 shadow">
    ...
    <script>
        $(document).ready(function() {
            $('#ordersTable').DataTable({
                "order": [
                    [0, "desc"]
                ], // order by first column (Order #) descending
                "pageLength": 10,
